...
public class RecursiveVisitor0 extends Visitor {

    public RecursiveVisitor0(Visitor nextVisitor) {
        super(nextVisitor);
    }
...
//        } else
    	// I only process those three, what is more, when executing the visitor statement,
    	// the replay==false and the runtime==false
...
//        }
    ...
    public void visitStmtAssign(SootMethod sm, Chain units, AssignStmt assignStmt) {
	    ...
	    if(assignStmt.toString().contains(Parameters.CARE_ANNOTATION))
	    {
	    	if(Parameters.isReplay)
	    		nextVisitor.addCallCaredByClap(sm, units, assignStmt);
		}
    }/*
